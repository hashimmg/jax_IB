...
@pytest.mark.parametrize("N", [128])
@pytest.mark.parametrize("axis", [0,1])
def test_ifft(N, axis, mesh_shape, axis_names):
  x = jnp.array(np.random.rand(N, N))
  mesh = jax.make_mesh(mesh_shape, axis_names)
  sharding = jax.sharding.NamedSharding(mesh, P(*axis_names))
  y = jax.device_put(x, sharding)
  jpfft = jax.jit(shmap(fft.ifft, mesh=mesh, in_specs = (P(*axis_names),None,None), out_specs = P(*axis_names)), static_argnums = (1,2))
  actual = np.array(jpfft(y, axis, axis_names[axis]))
  expected = np.array(jnp.fft.ifft(x, axis=axis))
  np.testing.assert_allclose(actual, expected)


@pytest.mark.parametrize("mesh_shape", [(1,8), (2,4), (4,2), (8,1)])
@pytest.mark.parametrize("N", [128])
def test_ifft_2d(N, mesh_shape, axis_names):
  x = jnp.array(np.random.rand(N, N))
  mesh = jax.make_mesh(mesh_shape, axis_names)
  sharding = jax.sharding.NamedSharding(mesh, P(*axis_names))
  y = jax.device_put(x, sharding)
  jpfft = jax.jit(shmap(fft.ifft_2d, mesh=mesh, in_specs = (P(*axis_names),None), out_specs = P(*axis_names)), static_argnums = (1,))
  actual = np.array(jpfft(y, axis_names))
  expected = np.array(jnp.fft.ifftn(x))
  np.testing.assert_allclose(actual, expected)
