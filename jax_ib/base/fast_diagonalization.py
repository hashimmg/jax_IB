...
def _hermitian_matmul_transform(
    ...
    eigenvalues, eigenvectors = zip(*map(jnp.linalg.eigh, operators))

    # Example: if eigenvalues=[a, b, c], then:
    #   summed_eigenvalues[i, j, k] == a[i] + b[j] + c[k]
    # for all i, j, k.
    summed_eigenvalues = functools.reduce(jnp.add.outer, eigenvalues)
...
def _circulant_fft_transform(
    ...
    def apply(rhs: Array) -> Array:
        if rhs.shape != shape:
            raise ValueError(f"rhs.shape={rhs.shape} does not match shape={shape}")
        # mganahl: see https://en.wikipedia.org/wiki/Circulant_matrix#Applications
...
def pseudo_poisson_inversion(
    ...
    def apply(rhs: Array) -> Array:
        return fft.ifft_2d(diagonal * fft.fft_2d(rhs, axis_names), axis_names).astype(
            dtype
        )
