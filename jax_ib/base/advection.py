...
def advect_linear(
    c: GridVariable, v: GridVariableVector, dt: Optional[float] = None
) -> GridArray:
...
def _align_velocities(v: GridVariableVector) -> Tuple[GridVariableVector]:
    ...
    aligned_v = tuple(
        tuple(interpolation.linear(v[i], offsets[i][j]) for j in range(grid.ndim))
    ...
    for i in range(ndim):
        for j in range(ndim):
            if i <= j:
                ...
                # bc = aligned_v[i][j].bc
                flux[i] += (
                    GridVariable(aligned_v[i][j].array * aligned_v[j][i].array, bc),
                )
            else:
                flux[i] += (flux[j][i],)
    return tuple(flux)
...
def advect_van_leer(c: GridVariable, v: GridVariableVector, dt: float) -> GridArray:
    """Computes advection of a scalar quantity `c` by the velocity field `v`.
...
def advect_step_semilagrangian(
    ...
    coords = [
        x - dt * interpolation.linear(u, c.offset).data
...
def stable_time_step(
    max_velocity: float, max_courant_number: float, grid: grids.Grid
) -> float:
