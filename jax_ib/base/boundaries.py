...
# TODO: the code currently allows ConstantBoundaryConditions to be time dependent.
...
class ConstantBoundaryConditions(BoundaryConditions):
    ...
    def __init__(
        ...
        object.__setattr__(
            self, "time_stamp", time_stamp if time_stamp is not None else []
        )
    ...
    def shift(
        ...
        padded = self._pad(
            u, offset, axis
        )  # mganahl: why not use simply jnp.roll for periodic boundary conditions?
    ...
    def _pad(
        ...
        if (
            not (bc_type == BCType.PERIODIC or bc_type == BCType.DIRICHLET)
            and abs(width) > 1
        ):
            raise ValueError(
        ...
        if bc_type == BCType.PERIODIC:
            ...
            if np.isclose(u.offset[axis] % 1, 0.5):  # cell center
                ...
                if u.grid.shape[axis] > u.shape[axis] + 1:
                    raise ValueError(
                        "For a dirichlet cell-face boundary condition, "
                ...
                if _needs_pad_with_boundary_value():
                    if np.isclose(abs(width), 1):
                        ...
                        data = 2 * jnp.pad(
                            ...
                            full_padding,
                            mode="constant",
                            constant_values=tuple(padding_values),
                        ) - jnp.pad(expanded_data, full_padding_past_bc, mode="reflect")
                    ...
                    data = 2 * jnp.pad(
                        ...
                        mode="constant",
                        constant_values=tuple(padding_values),
                    ) - jnp.pad(data, full_padding, mode="reflect")
            else:
                raise ValueError(
            ...
            if u.grid.shape[axis] > u.shape[axis]:
                raise ValueError("the GridArray shape does not match the grid.")
            if not (
                np.isclose(u.offset[axis] % 1, 0) or np.isclose(u.offset[axis] % 1, 0.5)
            ):
                raise ValueError(
                    "expected offset to be an edge or cell center, got "
                    f"offset[axis]={u.offset[axis]}"
                ...
                data = jnp.pad(data, full_padding, mode="edge") + u.grid.step[axis] * (
                    jnp.pad(data, full_padding, mode="constant")
                    - jnp.pad(
                        data,
                        full_padding,
                        mode="constant",
    ...
    def values(
        ...
        bc_values = tuple(
            jnp.full(
                grid.shape[:axis] + grid.shape[axis + 1 :], self.bc_values[axis][-i]
            )
            for i in [0, 1]
        )
        return bc_values
    ...
    def trim_boundary(self, u: grids.GridArray) -> grids.GridArray:
        ...
        for axis in range(u.grid.ndim):
            if (
                np.isclose(u.offset[axis], 0.0)
                and self.types[axis][0] == BCType.DIRICHLET
            ):
                u = self._trim(u, -1, axis)
            elif (
                np.isclose(u.offset[axis], 1.0)
                and self.types[axis][1] == BCType.DIRICHLET
            ):
                u = self._trim(u, 1, axis)
        return u

    def pad_and_impose_bc(
        self, u: grids.GridArray, offset_to_pad_to: Optional[Tuple[float, ...]] = None
    ) -> grids.GridVariable:
...
class HomogeneousBoundaryConditions(ConstantBoundaryConditions):
    ...
    def __init__(self, types: Sequence[Tuple[str, str]]):
        ...
        bc_fn = lambda x: x
        time_stamp = 0.0
        super(HomogeneousBoundaryConditions, self).__init__(
            time_stamp, values, types, bc_fn
        )
...
def _boundary_update(
    all_variable: particle_class.All_Variables, time_stamp_1, time_stamp_2
) -> particle_class.All_Variables:
    v = all_variable.velocity
    ...
    vx_bc = (
        (bcfn[0](time_stamp_1), bcfn[1](time_stamp_2)),
        (bcfn[2](time_stamp_1), bcfn[3](time_stamp_2)),
    )
    vy_bc = (
        (bcfny[0](time_stamp_1), bcfny[1](time_stamp_2)),
        (bcfny[2](time_stamp_1), bcfny[3](time_stamp_2)),
    )

    vel_bc = (
        ConstantBoundaryConditions(
            values=vx_bc, time_stamp=time_stamp_1, types=v[0].bc.types, boundary_fn=bcfn
        ),
        ConstantBoundaryConditions(
            values=vy_bc,
    ...
    return particle_class.All_Variables(
        v_updated, pressure, Drag, Step_count, MD_var, time
    )
...
def channel_flow_boundary_conditions(
    ...
    Returns:
      BoundaryCondition instance.
    """
    bc_type = ((BCType.PERIODIC, BCType.PERIODIC), (BCType.DIRICHLET, BCType.DIRICHLET))
...
def Moving_wall_boundary_conditions(
    ...
    return ConstantBoundaryConditions(
        values=bc_vals, time_stamp=time_stamp, types=bc_type, boundary_fn=bc_fn
    )
...
def find_extremum(fn, extrema, i_guess):
    if extrema == "maximum":
        ...
        raise ValueError(
            'No extrema was correctly identified. For maximum, type "maiximum". For minimization, type "minimum". '
        )
...
def periodic_and_dirichlet_boundary_conditions(
    bc_vals: Optional[Tuple[float, float]] = None, periodic_axis=0
) -> ConstantBoundaryConditions:
    """Returns BCs periodic for dimension 0 and Dirichlet for dimension 1.
    ...
    if periodic_axis == 0:
        if not bc_vals:
            ...
            return ConstantBoundaryConditions(
                (periodic, dirichlet), ((None, None), bc_vals)
            )
    else:
        if not bc_vals:
            ...
            return ConstantBoundaryConditions(
                (dirichlet, periodic), (bc_vals, (None, None))
            )
...
def get_advection_flux_bc_from_velocity_and_scalar(
    ...
    for axis in range(c.grid.ndim):
        if u.bc.types[axis][0] == "periodic":
            ...
            u.bc.types[axis][0] == BCType.DIRICHLET
            and u.bc.types[axis][1] == BCType.DIRICHLET
            and u.bc.bc_values[axis][0] == 0.0
            and u.bc.bc_values[axis][1] == 0.0
        ):
            flux_bc_types.append((BCType.DIRICHLET, BCType.DIRICHLET))
        else:
            raise NotImplementedError(
...
def new_periodic_boundary_conditions(
    ...
    Returns:
      BoundaryCondition instance.
    """
    bc_type = ((BCType.PERIODIC, BCType.PERIODIC), (BCType.PERIODIC, BCType.PERIODIC))
    for _ in range(ndim - 2):
        bc_type += ((BCType.PERIODIC, BCType.PERIODIC),)
