...
def Update_particle_position_Multiple_and_MD_Step(step_fn, all_variables, dt):
    ...
    U0 = dx_dt(current_t)
    # print(U0)
    Newparticle_center = jnp.array(
        [particle_centers[:, 0] + dt * U0[0], particle_centers[:, 1] + dt * U0[1]]
    ).T
...
def Update_particle_position_Multiple(all_variables, dt):
    ...
    velocity = all_variables.velocity
    current_t = velocity[0].bc.time_stamp
    # maganahl: I think this was a bug (to confirm with Aramco)
    # the following code now updates the particle_centers of all particles, as it should
    pressure = all_variables.pressure
    Step_count = all_variables.Step_count + 1
    MD_var = all_variables.MD_var
    return pc.All_Variables(velocity, pressure, Drag, Step_count, MD_var)
