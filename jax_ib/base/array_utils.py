...
def slice_along_axis(
    inputs: PyTree, axis: int, idx: Union[slice, int], expect_same_dims: bool = True
) -> PyTree:
...
def split_along_axis(
    inputs: PyTree, split_idx: int, axis: int, expect_same_dims: bool = True
) -> Tuple[PyTree, PyTree]:
...
def split_axis(
    inputs: PyTree, axis: int, keep_dims: bool = False
) -> Tuple[PyTree, ...]:
...
def block_reduce(
    array: Array, block_size: Tuple[int, ...], reduction_fn: Callable[[Array], Array]
) -> Array:
    ...
    for b, s in zip(block_size, array.shape):
        ...
        if residual != 0:
            raise ValueError(
                "`block_size` must divide `array.shape`;"
...
def _laplacian_boundary_dirichlet_cell_centered(
    laplacians: List[Array], grid: grids.Grid, axis: int, side: str
) -> None:
...
def laplacian_matrix_w_boundaries(
    ...
    for axis in range(grid.ndim):
        ...
        if np.isclose(offset[axis] % 1, 0.0):
            if (
...
def gram_schmidt_qr(
    matrix: Array, precision: jax.lax.Precision = jax.lax.Precision.HIGHEST
) -> Tuple[Array, Array]:
    ...
    for vec_index, column in enumerate(columns):
        ...
        r_rows.append(
            jnp.asarray(
                [
                    jnp.dot(columns[i], next_q_column) if i >= vec_index else 0.0
...
def interp1d(  # pytype: disable=annotation-type-mismatch  # jnp-type
    ...
    if isinstance(fill_value, str):
        ...
        if fill_value.ndim > 0:
            raise ValueError(
                f"Only scalar `fill_value` supported. "
    ...
    def interp_func(x_new: jax.Array) -> jax.Array:
        ...
        above_idx = jnp.minimum(
            n_x - 1, jnp.searchsorted(x, x_new_clipped, side="right")
        )
