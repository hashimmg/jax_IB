...
def setup_variables(grid):
    ...
    velocities = tuple(
        [
            grids.GridVariable(
                grid.eval_on_mesh(
                    fn=lambda x, y: jnp.sin(x + y) * jnp.cos(x - y), offset=offset
                ),
                bc,
            )
    ...
    pressure = grids.GridVariable(
        grid.eval_on_mesh(fn=lambda x, y: jnp.cos(x + y), offset=grid.cell_center),
...
def test_pressure_projection_sharded(N, mesh_shape, axis_names):
    ...
    v_projected_exp, new_pressure_exp = prs.projection_and_update_pressure(
        pressure, velocities
    )
    ...
    def wrapper(pressure, velocities, laplacian_eigenvalues, width):
        ...
        v_projected, new_pressure = prs.projection_and_update_pressure_sharded(
            local_pressure, local_velocities, pinv, width
        )
