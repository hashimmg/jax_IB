...
def _fft1d_i(array: jax.Array, axis_name: str)-> jax.Array:
    ...
    N = M * array.shape[0]
    row = jnp.expand_dims(jnp.exp(-2*jnp.pi*1j/N*i*(N//M) * jnp.arange(N//M)), axis=1)
    result = jnp.zeros_like(array)
    for j in range(M):
        column = jnp.expand_dims(jnp.exp(-2*jnp.pi*1j/N*((j + i) % M) * (N//M) * jnp.arange(N//M)), axis=1)
        phase = jnp.exp(-2*jnp.pi*1j/N * i * ((j + i) % M) * (N//M)**2)
        result += column * jnp.fft.fft(row *array, n = N, axis=0)[:(N//M),:]* phase
...
def _fft1d_j(array: jax.Array, axis_name: str)->jax.Array:
    ...
    `array` should be a jax.Array sharded on a device mesh ('i','j')
    `axis_name` is the name of the second sharding axis, i.e. `'j'` for
    the above mesh
    ...
    M = jax.lax.psum(1, axis_name=axis_name)
    N = M * array.shape[1]
    column = jnp.expand_dims(jnp.exp(-2*jnp.pi*1j/N*j*(N//M) * jnp.arange(N//M)), axis=0)
    result = jnp.zeros_like(array)
    for i in range(M):
        row = jnp.expand_dims(jnp.exp(-2*jnp.pi*1j/N*((j + i) % M) * (N//M) * jnp.arange(N//M)), axis=0)
        phase = jnp.exp(-2*jnp.pi*1j/N * j * ((j + i) % M) * (N//M)**2)
        result += row * jnp.fft.fft(array*column, n = N, axis=1)[:, :(N//M)]* phase
        array  = jax.lax.ppermute(array, axis_name=axis_name, perm = [(k, (k-1)%M) for k in range(M)])
    return result
