...
def solve_linear_sharded(
    velocities: tuple[GridVariable], pinv: callable, width: int
) -> grids.GridArray:
    """
...
def projection_and_update_pressure_sharded(
    ...
    velocities: tuple[grids.GridVariable],
    pinv: callable,
    width: int,
    ...
    solution = grids.GridVariable(
        solve_linear_sharded(velocities, pinv, width), pressure_bc
    )
    new_pressure_array = grids.GridArray(
        solution.data + pressure.data, pressure.offset, pressure.grid, pressure.width
    )
    new_pressure = grids.GridVariable(new_pressure_array, pressure_bc)

    grads = tuple(
        [g.crop(width) for g in fd.forward_difference(solution.shard_pad(width))]
    )
    grads = tuple([grids.GridVariable(g, solution.bc) for g in grads])

    v_projected = tuple(
        grids.GridVariable(u.array - g.array, u.bc) for u, g in zip(velocities, grads)
    )
...
def projection_and_update_pressure(
    ...
    New_pressure_Array = grids.GridArray(
        qsol.data + pressure.data, qsol.offset, qsol.grid, pressure.width
    )
    New_pressure = grids.GridVariable(New_pressure_Array, pressure_bc)
